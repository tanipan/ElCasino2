...
<div class="card card-row card-secondary" style="width: {{ $anchura }}px !important">
    ...
    <div class="card-body">
        @foreach ($orders as $order)
            @if (
                ($order->hide_in_burger == '0000-00-00 00:00:00' or $order->hide_in_burger == null) and
                    $order->getType() == 'table' and
                    count($order->getLinesBurger()) and
                    $order->orderCanBePrepared())
                @php
                    $pedido_listo = $order->getLinesBurgerIsPrepared();
                @endphp
                ...
                <div class="card card-info card-outline"
                    ...
                    <div class="card-body">
                        ...
                        @foreach ($order->getLinesBurger() as $line)
                            <div class="custom-control custom-checkbox"
                                style="font-size: {{ $titulo_plato }}px;margin-bottom:10px;cursor:pointer">
                                ...
                                @if ($line->units_in_progress == $line->units)
                                    ...
                                    <label data-line-id="{{ $line->id }}" class="entrante custom-control-label"
                                        style="margin-bottom: 8px">
                                    </label>
                                @endif
                                <span class="entrante_fuego" style="font-weight: bold"
                                    data-line-id="{{ $line->id }}">

                                    <span style="{!! $line->units_in_progress == $line->units ? 'text-decoration: line-through;' : '' !!}">
                                        @if ($line->units > 1)
                                            <span
                                    ...
                                    @for ($i = 0; $i < $line->units_in_progress; $i++)
                                        ✅
                                    @endfor

                                    @foreach ($line->getElements(1) as $ele)
                                        <!-- Elementos -->
