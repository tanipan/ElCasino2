    ...
    <table class="table">
        ...
        <tbody>
            ...
            @foreach ($cestaHumana as $k => $elemento)
                @php
                    $importe_actual = ($elemento['precio'] / $elemento['unidades']) * ($elemento['unidades'] - ($elemento['unidadesPagar'] + $elemento['unidadesPagadas']));
                @endphp
                ...
                <tr>
                    ...
                    <td width="20%" class="table_center" style="vertical-align: middle">
                        <b
                            style="padding-left: 3px; padding-right: 3px">{{ $elemento['unidades'] - ($elemento['unidadesPagar'] + $elemento['unidadesPagadas']) }}</b>
                        <a href="#">
                            <i style="font-size: 24px" class="fa mas2" data-plato-id="{{ $elemento['plato_id'] }}"
                                data-line-id="{{ $elemento['idLinea'] }}">✅</i>
                        </a>
...
<script>
    $(".mas2").click(function() {
        addPagar($(this).attr("data-plato-id"), $(this).attr("data-line-id"));
    });

    function addPagar(plato, idLinea) {
        $.get("{{ route('front.addPagar') }}", {
                plato: plato,
                linea: idLinea
            },
